...
// Built-in Diagnostic Test (Visit /api/contact.php in browser to verify server mail health)
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['test'])) {
        ...
        $subject = "RR Infra Mail Server Diagnostic Test - " . date('Y-m-d H:i:s');
        $body = "This is a diagnostic test email from https://rrinfra.co.in/api/contact.php to verify server mail delivery.";
...
// 1. SAVE LEAD TO LOCAL BACKUP FILE (Ensures no inquiry is ever lost)
$leadEntry = [
...
// Additional sendmail parameters (-f sets Return-Path/envelope sender)
$extraParams = "-f" . $senderEmail;

// Attempt mail dispatch
$mailSent = @mail($toEmail, $subject, $emailBody, $headers, $extraParams);

// If first attempt failed, retry without extra parameters
if (!$mailSent) {
    $mailSent = @mail($toEmail, $subject, $emailBody, $headers);
}

// Always return success since the lead is securely saved in leads_backup.json
echo json_encode([
    'success' => true,
